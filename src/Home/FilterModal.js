...
import "react-input-range/lib/css/index.css"; //Importing CSS file for imput range styling
...
const FilterModal = ({selectedFilters,onFilterChange,onClose}) => {
    ...
    //useEffect hook to update states 
    useEffect(()=>{
        setPriceRange({
            min: selectedFilters.priceRange?.min || 600,
            max: selectedFilters.priceRange?.max || 30000,
    ...
    const handleClearFilters=()=>{
        setPriceRange({min:600, max:30000})//reset price
        setPropertyType("")
        setRoomType("")
        setAmenities([])
    }
    ...
    const handleAmenitiesChange=(selectedAmenity)=>{
        setAmenities((prevAmenities)=> prevAmenities.includes(selectedAmenity)
        ?prevAmenities.filter((item)=> item!==selectedAmenity):
        [...prevAmenities,selectedAmenity]);
    }


    //Fution to handle changes in property type
    ...
    //Fution to handle changes in room type
  ...
  return (
    <div className='modal-backdrop'>
        <div className='modal-content'>
            ...
            {/* Filter Section */}
            <div className='modal-filters-containers'>
                ...
                <div className='filter-section'>
                    ...
                    <div className='amenity-checkboxes'>
                        {amenitiesOptions.map((option)=>(
                            <div key={option.value} className="menities-checkbox">
